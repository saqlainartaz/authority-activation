...
| --- | --- | --- | --- | --- |
| Approved audience choice, Onboarding | Save the design's audience answer | Backend onboarding schema/options and `PUT /v1/onboarding` at `6a7429f` | The old backend uses a different authoritative audience catalogue. The approved choice stays local and the screen says so. | Product decision plus compatible option mapping or backend catalogue change |
| Guardrail answer, Onboarding completion | Supply every backend-required answer | Backend readiness/completion validation | The approved onboarding has no “never say”/guardrail question. Compatible answers are retained, completion is refused truthfully, and unsaved UI input stays visible. | Add an approved guardrail question or change the backend requirement through an approved contract change |
...
| Training questions, Train your AI | Persist design-specific answers | Onboarding/atom/profile contracts | No backend model matches these question IDs/types. Answers stay browser-local. | Stable server question schema and tenant-scoped answer endpoints |
