...
Evidence baseline: stakeholder frontend at initial revision `ec2f05f...`, previous BFF/admin/agent at `8c790eb...`, compatible Python backend at `6a7429f...`. “Verified” means local synthetic checks against that isolated backend, not a live provider.
...
| Onboarding read/write | `/api/client/onboarding` → backend onboarding answers and completion | Atomic server write; retained UI input on refusal | Partially integrated; exact catalogue/guardrail gaps documented |
...
| Training questions | Approved static question model and browser persistence | Browser local storage only | Demo/local; documented |
...
| Knowledge/evidence | `/api/client/atoms`, constraints routes, chat variant-source reads, and `/api/client/content-items/[id]/versions` receipts | Backend atoms, decisions, snapshots, receipt claim spans and citations | Integrated; selected-version receipt offsets now drive the evidence lens and citation popovers; Unicode-safe mapping is unit-tested |
| Agent conversation | `/api/client/chat/sessions/*`; Next Node TypeScript agent calls context/draft tools; authenticated onboarding/atom overview only for identity, data and delegated-topic questions | Backend transcript/session; SSE presentation; server-verified draft; server-derived account facts | Integrated; clarification, concise/delegated-topic policy, reload, streaming, retry-safe ids, citations exercised |
...
| Stop/cancel | Browser aborts the open SSE display | Backend has no cancellation signal/endpoint; a turn may finish server-side | Client-side only; UI says so explicitly |
