...
export default function Workspace() {
  ...
  return (
    <div className="relative flex min-h-0 flex-1 flex-col bg-background text-foreground">
      ...
      <Dialog open={rename} onOpenChange={setRename}><DialogContent><DialogHeader><DialogTitle>Rename post</DialogTitle><DialogDescription>A title to find this post in your Library.</DialogDescription></DialogHeader><Input autoFocus aria-label="Post title" value={title} onChange={e => setTitle(e.target.value)} onKeyDown={e => { if (e.key === 'Enter' && title.trim()) void ws.rename(title).then(saved => { if (saved) setRename(false); }); }} /><DialogFooter><Button variant="outline" onClick={() => setRename(false)}>Cancel</Button><Button disabled={!title.trim()} onClick={() => { void ws.rename(title).then(saved => { if (saved) setRename(false); }); }}>Save title</Button></DialogFooter></DialogContent></Dialog>
      {mobile ? (
