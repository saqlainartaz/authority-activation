...
export default function AgentThread({ ws, inlineDraft = false, draft }: { ws: Workspace; inlineDraft?: boolean; draft?: ReactNode }) {
  ...
  return <DraftResult.Provider value={draft}><AssistantRuntimeProvider runtime={runtime}><ThreadPrimitive.Root className="rf-native-thread"><ThreadPrimitive.Viewport autoScroll={false} scrollToBottomOnRunStart={false} scrollToBottomOnInitialize={false} scrollToBottomOnThreadSwitch={false}><ThreadPrimitive.Messages components={{ UserMessage, AssistantMessage }} /></ThreadPrimitive.Viewport></ThreadPrimitive.Root></AssistantRuntimeProvider></DraftResult.Provider>;
}
